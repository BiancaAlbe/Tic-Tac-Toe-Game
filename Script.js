window.onload = function gridGenerate() {
    for (var i = 0; i < 3; ++i) {
        for (var j = 0; j < 3; ++j) {
            document.getElementById('test1').innerHTML += "<div id=table1(" + i + "," + j + ") onclick = table_clicks("+ i +","+ j +") class = click ></div>";
        }
...
function table_clicks(i, j) {
    ...
    let afisare = array_verify(square);
    console.log (afisare);
    if (afisare) {
        document.getElementById('mesage').innerText = afisare;
    }
}
